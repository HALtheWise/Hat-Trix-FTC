...
void getEvents()
{
	...
	for( int i=1; i <= 12; i++ )
	{
		...
		addEvent(i, (joystick.joy1_Buttons & (1 << i)) ? B_PRSD : B_RLSD); //acting(Bmap[0][i]);

		if((joystick.joy2_Buttons & (1 << i)) != (PrevJoy.joy2_Buttons & (1<< i)))
		addEvent(i+10, (joystick.joy2_Buttons & (1 << i)) ? B_PRSD : B_RLSD);//acting(Bmap[1][i]);
	}
