...
void mot2(int leftPow, int rightPow){ //Takes left an right powers, applies them to wheels
...
float transformPower(float input){
	float output;
	int sign = sgn(input);
	input = abs(input);
	if (input > 100){ input = 100;}
	if (input <= 5)				{output = 4*input;} 									// 0...5    --> 0 ...20
	else if (input <= 70)	{output = (input - 5)*(40.0/65)+20;}	// 5...70   --> 20...60
	else 									{output = (input - 70)*(40.0/30)+60;}	// 70...100 --> 60...100
	return output*sign;
}

//TODO: Treat turning and straight driving seperately
void vmot(float leftPow, float rightPow){
	mot2(transformPower(leftPow), transformPower(rightPow));
}
...
void moveTo (FieldPos target, int power, DrivingDirection forward, float aggressiveness)
{
	...
	float timelimit = distanceBetween(robot, target)*50+200; // set a time limit based on distance
	...
	while(abs(neededTurn(target, forward)) <= PI/2)//  toGo > OCD)
	{
		...
		turnCorrection *= ((toGo)/20.0); //Additional multiplier ranges from 0 to ~10, making late drive less jaggy
