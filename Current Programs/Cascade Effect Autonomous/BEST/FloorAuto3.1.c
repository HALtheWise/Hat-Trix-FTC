...
void floorStart(){
	...
	if (mode == MODE_MEDIUM_ALWAYS || mode == MODE_DEFEND_CENTER_MEDIUM || mode == MODE_SMART_CENTER_MEDIUM){
		...
		if (backsideNavigate){ //Backside navigation
			...
			bool madeIt = turnAndMoveTo(GPS_mediumGoalPosition, speed_fast, Backward);
			if (!madeIt){ //Try to avoid a robot in our way.
				turnAndMoveTo(GPS_navPoint2, speed_normal, AutomaticDirection);
				turnAndMoveTo(GPS_navPoint3, speed_normal, AutomaticDirection);
				turnAndMoveTo(GPS_navPoint4, speed_normal, AutomaticDirection);
