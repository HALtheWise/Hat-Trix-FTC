...
#pragma config(Servo,  srvo_S3_C1_1,    deliver,              tServoStandard)
...
task main()
{
	...
	move(-70 , 40, 1, 1);
	ClearTimer(T1);
	while(nMotorEncoder[arm] > -4500 && time1[T1] < 3000)
	{
		writeDebugStreamLine("arm encoder value: %d" , nMotorEncoder[arm] );
		motor[arm] = -70;
	}
	servo[deliver] = 30;
	wait1Msec(3000);
	/*ClearTimer(T1);
	while(nMotorEncoder[arm] > -800 && time1[T1] < 2000)
	motor[arm] = -30;
	motor[arm] = 0;
	*/
	//move(-600, 30);
