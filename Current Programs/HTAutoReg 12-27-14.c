...
int kickSeeker(Signls *sigList)
{
	...
	if (!HTIRS2readAllACStrength(seeker, acS1, acS2, acS3, acS4, acS5 ))
		...
		insert(sigList, acS1, 1);
		insert(sigList, acS2, 2);
		insert(sigList, acS3, 3);
		insert(sigList, acS4, 4);
		insert(sigList, acS5, 5);
	...
	if(glide != 1)
	{
		while(abs(nMotorEncoder[FrontR]) < abs(deg))
		{
			...
			else
			{
				...
				float lFrG = (sigList->str[0]*sin(ang))/3;
				float fWbK = (sigList->str[0]*cos(ang))/3;
