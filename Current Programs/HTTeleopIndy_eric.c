...
#pragma config(Motor,  motorA,          extend1,       tmotorNXT, openLoop)
#pragma config(Motor,  motorB,          extend2,       tmotorNXT, openLoop)
...
{
	const int ARM_UP_POSITION = 800;
	const int ARM_DOWN_POSITION = 50;
	const int ARM_UP_POWER = 50;
...
{
	const int OPEN_POSITION = 81;
	const int CLOSED_POSITION = 93;
	const int open = 0;
	const int closed = 1;
...
{
	static bool typeDrive = false; //true for tank drive, false for steering drive
	static float lefty, righty;

	const float accelLimit = 300; //Limits the maximum change in wheel power per second. Units are "percent/second". 0 to disable.
	...
	case C_EVENT:
		...
		if(get_owner(RES_WHEELS) == AI_DRIVE)
		{
			...
			if (accelLimit != 0){ //Acceleration limiting code
				static float lastLeft = 0.0, lastRight = 0.0;
				static int lastTime = nPgmTime;

				float dt = (nPgmTime - lastTime) / 1000.0; //Time since last loop iteration, in seconds
				lastTime = nPgmTime;
				if (dt > 0.0 && dt < 1.0) {

					float dLeft = lefty - lastLeft; //Units of "%"
					float dRight = righty - lastRight;

					if (abs(dLeft/dt) > accelLimit){
						lefty = lastLeft + sgn(dLeft) * accelLimit * dt;
					}
					if (abs(dRight/dt) > accelLimit){
						righty = lastRight + sgn(dRight) * accelLimit * dt;
					}
					lastLeft = lefty;
					lastRight = righty;
				}
