...
int takeUpTheSlack(int gobutton, int stopbutton)
{
	static int Stop = 1;
...
int readyToHang(int temp, int stopbutton)
{
	static int Stop = 1;
	static int Stage = 0;
	...
	if(Stage == 0)
	{
		writeDebugStreamLine("Stage 0 is running");
		...
		{
			writeDebugStreamLine("Stage 1 = PERFECTO");
			motor[arms] = 0;
			Stage++;
			ClearTimer(T2);
		}
		...
		if(time1[T2] >= 5000)
		{
			writeDebugStreamLine("timed out (arms)");
			Stop = 1;
			return 1;
		}
	...
	{
		writeDebugStreamLine("Stage 1 is running");
		if(nMotorEncoder[wrist] >= -760)
			motor[wrist] = -15;
		else if(nMotorEncoder[wrist] <= -800)
			motor[wrist] = 15;
		else
		{
			writeDebugStreamLine("Stage 1 = PERFECTO");
		...
		if(time1[T2] >= 2000)
		{
			writeDebugStreamLine("timed out (wrist)");
			Stop = 1;
			return 1;
		}
	}
	...
	{
		writeDebugStreamLine("Stage 2 is running");
		motor[hookWinch] = -100;
		if(SensorValue[light] <= 25 && time1[T3] > 100)
		{
			writeDebugStreamLine("rotated %d", rotationCount);
			writeDebugStreamLine("light reading : %d" , SensorValue[light]);
			rotationCount++;
			ClearTimer(T3);
		}
		if(rotationCount >= 7)
		{
			writeDebugStreamLine("finished! %d", rotationCount);
			motor[hookWinch] = 0;
			Stage = -1;
			Stop = 1;
			ClearTimer(T2);
		...
		if(time1[T2] >= 5000)
		{
			writeDebugStreamLine("timed out (hookWinch)");
			Stop = 1;
			return 1;
		}
	}
...
task main()
{
	...
	while(1)
	{
		...
		//writeDebugStreamLine("the joy2Btn(4) = %d", joy2Btn(4));
		if(readyToHang(tempint, cancel))
		{
			if(takeUpTheSlack(finalStand, cancel))
			{
